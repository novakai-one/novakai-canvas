  ...
  return (
    <section className="bench-draft" data-selected={selected}>
      <header className="bench-draft__header">
        ...
        <button type="button" className="nodrag" onClick={data.cancel} aria-label="Cancel new conversation">
          ×
        </button>
      </header>
      <div className="bench-draft__agents nodrag nowheel">
        {data.agents.length > 0 ? data.agents.map((agent) => (
          <button key={agent.id} type="button" className="nodrag" onClick={() => data.accept(agent)}>
