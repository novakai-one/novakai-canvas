...
import { useStore, makeRecord, sessionId } from '../../app/store';
...
function AttentionRow({ item, elected }: { item: AttentionItem; elected: boolean }) {
  const { select, selected, graph, patch, addRecord, enterRoom } = useStore();
  const openTarget = graph.get(item.openId);
  const room = openTarget ? roomFor(openTarget) : null;
...
export function CommandCenter() {
  const { feed, elected, graph } = useStore();
          ...
          return (
            <section className="command-center__group" key={reason}>
              ...
              <div className="command-center__rows">
                {items.map((item) => (
                  <AttentionRow key={item.id} item={item} elected={elected?.id === item.id} />
                ))}
              </div>
            </section>
          );
