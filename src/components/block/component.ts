...
const SYNTAX = 'block "label" [font=… size=… weight=… align=… text=… background=… border-color=… border=… radius=… padding=…]';
const EXAMPLE = 'block "Refusal rate" size=14 weight=600 align=center text=green border-color=green border=1 radius=8 padding=12';
...
export function layoutBlockText(
...
): { size: Size; lines: readonly string[] } {
  const sourceLines = [label, ...authoredLines];
  const contentWidth = Math.max(1, Math.min(
    MAX_CONTENT_WIDTH,
    Math.ceil(Math.max(...sourceLines.map((line) => textWidth(line, appearance)))),
  ));
  const lines = sourceLines.flatMap((line) => wrapText(line, contentWidth, appearance));
  ...
  return {
    size: {
      ...
      height: Math.ceil(lines.length * lineHeight + inset * 2),
    },
    lines,
  };
}
...
export const blockComponent: DiagramComponent<'block'> = {
  ...
  renderSvg(node: CanvasNode, box, appearance) {
    ...
    layout.lines.forEach((line, index) => {
      parts.push(`<text x="${x}" y="${box.y + inset + appearance.fontSize + index * lineHeight}" fill="${appearance.textColor}" font-family="${escapeXml(appearance.fontFamily)}" font-size="${appearance.fontSize}" font-weight="${appearance.fontWeight}" text-anchor="${anchor}" xml:space="preserve">${escapeXml(line)}</text>`);
    });
    return parts.join('\n');
  },
};
