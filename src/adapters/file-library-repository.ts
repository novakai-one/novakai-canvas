import type { CanvasLibraryRepository, WriteOutcome } from '../application/canvas-library.ts';
import { diagramRecordSchema, libraryIndexSchema } from '../domain/record-schema.ts';
 ...
 */
export function createFileLibraryRepository(base = '/api'): CanvasLibraryRepository {
  ...
  return {
    readIndex: () => readValidated(indexEndpoint, libraryIndexSchema),

    writeIndex: (index, expectedRevision) => writeValidated(
      indexEndpoint, libraryIndexSchema, index, expectedRevision,
    ),

    readDiagram: (id) => readValidated(diagramEndpoint(id), diagramRecordSchema),

    writeDiagram: (record, expectedRevision) => writeValidated(
      diagramEndpoint(record.id), diagramRecordSchema, record, expectedRevision,
    ),
