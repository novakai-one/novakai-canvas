...
export function renderSnapshotWires(scene: SnapshotScene): string[] {
  ...
  for (const wire of scene.wires) {
    ...
    parts.push(
      `<polyline points="${points.map((point) => `${point.x},${point.y}`).join(' ')}" fill="none" stroke="${appearance.strokeColor}"${appearance.dashArray ? ` stroke-dasharray="${appearance.dashArray}"` : ''} stroke-width="${appearance.strokeWidth}" marker-end="url(#${markerId})"/>`,
